                ...
                <nav style={{display:"flex",flexDirection:"column",gap:3,marginBottom:18}}>
                  {navItems.map((item,idx)=>(
                    <button key={idx} onClick={()=>navigate(item.path)} className="rpt-nav-btn">
                      <span style={{width:26,height:26,borderRadius:7,background:"rgba(255,255,255,0.06)",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:13,flexShrink:0}}> <Ic d={item.icon} size={14} />
                         </span> 
                      {item.label}
                    </button> 
                  ))}
