...
export default function FileLibraryPage() {
  ...
  const navItems = [
    ...
    { label:"File Library", path:"/file-library",         icon:"📁" },
          ...
          {menuOpen && (
            <div className="fl-sidebar-inner">
              ...
              <nav style={{ display:"flex",flexDirection:"column",gap:4,marginBottom:24 }}>
                {navItems.map((item,i) => (
                  <button key={i} className="fl-nav-item" onClick={() => navigate(item.path)}>
                    <span className="fl-nav-icon">{item.icon}</span>{item.label}
                  </button>
                ))}
              </nav>
